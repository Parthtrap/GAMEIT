...
function Sidebar() {
  ...
  async function fetchLiked() {
    const userSelector = JSON.stringify({ email: auth.user.email });
    const response = await fetch(
  ...
  return <div className="hidden p-1 mt-16 text-white bg-black md:block md:w-1/4">
    {auth.isLoggedIn ? <div>
      <h1>Liked Communities</h1>
